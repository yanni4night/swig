 ...
 *
 * @deprecated I have to remove this filter because Django syntax receives one parameter only
 */
 ...
 */
exports.slice = function (input, range){
  console.log(range);
  if (!range || ('string' !== typeof range && 'number' !== typeof range)) {
    throw new Error('parameter is required');
  }
  var se = String(range).split(':');
  ...
  if (utils.isArray(input) || 'string' === typeof input) {
    return input.slice(+se[0], +se[1]);
  }else{
    return input;
  }
};
